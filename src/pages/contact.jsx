...
const WA_GREETINGS = {
  en: 'Hi Brayan! I found your portfolio at brayanmurcia.lat and I would like to discuss a potential opportunity.',
  es: 'Hola Brayan! Encontre tu portafolio en brayanmurcia.lat y me gustaria conversar sobre una oportunidad.',
};

const getSocialLinks = (isEs) => [
  {
    id: 'whatsapp',
    icon: () => <WhatsAppSvg />,
    label: '+57 322 312 1409',
    href: `https://wa.me/573223121409?text=${encodeURIComponent(isEs ? WA_GREETINGS.es : WA_GREETINGS.en)}`,
    external: true,
  },
  {
    id: 'email',
    icon: () => <Mail size={18} color="#202124" />,
...
const ExecutiveSocialBento = ({ isEs }) => {
  const [copied, setCopied] = useState(null);
  const links = getSocialLinks(isEs);
    ...
    <div style={s.socialGrid}>
      {links.map((link) => (
        <motion.div key={link.id} style={s.socialCard} whileHover={{ y: -2, boxShadow: '0 4px 16px rgba(0,0,0,0.07)' }}>
          ...
          {(link.id === 'email' || link.id === 'whatsapp') && (
            <button
              style={s.copyBtn}
              onClick={(e) => {
                e.preventDefault();
                handleCopy(
                  link.id === 'email' ? 'abytecorp@gmail.com' : '+573223121409',
                  link.id
                );
              }}
            >
              {copied === link.id ? <Check size={13} color="#22c55e" /> : <Copy size={13} color="#9ca3af" />}
            </button>
          )}
        </motion.div>
      ))}
        ...
        <div className="contact-grid-responsive" style={s.grid}>
          ...
          <FadeInView delay={0.1}>
            <div style={s.leftCol}>
              ...
              <ExecutiveSocialBento isEs={isEs} />
            </div>
          </FadeInView>
